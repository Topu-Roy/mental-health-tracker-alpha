...
import { Emotion, EmotionTally } from '@/types/journal';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from '@/components/ui/card';
import { Label } from '@/components/ui/label';
import { Textarea } from '@/components/ui/textarea';
import { Slider } from '@/components/ui/slider';
import { Smile, Frown, Meh, ThumbsUp, Heart, CloudRain, Zap, Coffee, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export function ReflectionWizard({ onComplete, onCancel }: ReflectionWizardProps) {
  ...
  const [generalMood, setGeneralMood] = useState<Emotion | null>(null);
  const [emotionTallies, setEmotionTallies] = useState<Record<Emotion, number>>({
    'Happy': 0, 'Excited': 0, 'Grateful': 0, 'Relaxed': 0,
    'Sad': 0, 'Anxious': 0, 'Angry': 0, 'Tired': 0,
    'Frustrated': 0, 'Confused': 0, 'Proud': 0, 'Hopeful': 0
  });
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {EMOTIONS.map(({ label, icon }) => (
                  <div key={label} className="flex items-center justify-between p-3 border rounded-lg bg-card/50">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {Object.values(emotionTallies).every(c => c === 0) && <span className="text-muted-foreground italic">None recorded</span>}
                </div>
              </div>
